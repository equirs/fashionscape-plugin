...
[![Active Installs](http://img.shields.io/endpoint?url=https://api.runelite.net/pluginhub/shields/installs/plugin/fashionscape)](https://runelite.net/plugin-hub/show/fashionscape)
[![Plugin Rank](http://img.shields.io/endpoint?url=https://api.runelite.net/pluginhub/shields/rank/plugin/fashionscape)](https://runelite.net/plugin-hub/show/fashionscape)
...
* The randomizer now has a **"Cursed"** setting, which inverts the colour scoring to give you the most mismatched
  combinations it can.
...
* Equipment slots now rely directly on data derived from the cache instead of the item stats API / the RuneLite wiki 
  scraper: what this means for you is that there should be more options in your searches now. Many
...
* A bunch of data that was previously bundled with the plugin now lives in
  a [separate repo](https://github.com/equirs/fashionscape-data). There's a chance the requests retrieving this info fail. In some cases, GitHub might be
  down, in which case, the plugin will still work, but you may have to manually clear out some of your base models
  (you can do this by right-clicking the base model's icon). Idle animations and colour matching will simply not work
